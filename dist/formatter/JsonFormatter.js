...
class JsonFormatter {
    format(callGraph, options = { format: 'json' }) {
        const output = {};
    ...
    calculateDepthDistribution(callGraph) {
        const { nodes, edges, entryPointId } = callGraph;
        const depths = new Map();
        const visited = new Set();
        // BFS to calculate depths
        const queue = [{ nodeId: entryPointId, depth: 0 }];
     ...
     */
    formatWithSchema(callGraph, schema) {
        switch (schema) {
            case 'simple':

...
class MermaidFormatter {
    ...
    format(callGraph, options = { format: 'mermaid' }) {
    ...
    generateNodeDefinitions(nodes, entryPointId) {
        const lines = [];
    ...
    generateEdgeDefinitions(edges, nodes) {
        const lines = [];
    ...
    generateStyleDefinitions(nodes, entryPointId) {
        const lines = [];
        // Define style classes
        lines.push('    %% Styling');
        lines.push('    classDef entryPoint fill:#e1f5fe,stroke:#01579b,stroke-width:3px,color:#000;');
        lines.push('    classDef asyncFunction fill:#f3e5f5,stroke:#4a148c,stroke-width:2px,color:#000;');
        ...
        lines.push('    classDef arrowFunction fill:#f1f8e9,stroke:#33691e,stroke-width:1px,color:#000;');
        lines.push('');
        ...
        nodes.forEach(node => {
            ...
            if (!safeId || node.id === entryPointId)
                return;
            if (node.async) {
    ...
    getNodeShape(node, isEntryPoint) {
        if (isEntryPoint) {
     ...
     */
    formatAsSequenceDiagram(callGraph) {
        ...
        const addCallSequence = (nodeId, depth = 0) => {
            if (depth > 5)
                return; // Prevent infinite recursion
            const outgoingEdges = edges.filter(edge => edge.source === nodeId &&
                !processedEdges.has(edge.id));
            outgoingEdges.forEach(edge => {
                processedEdges.add(edge.id);
                const sourceNode = nodes.find(n => n.id === edge.source);
                const targetNode = nodes.find(n => n.id === edge.target);
                if (sourceNode && targetNode) {
                    const sourceParticipant = edge.source === entryPointId ?
                        'Entry' :
    ...
    validate(mermaidString) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (line.startsWith('classDef '))
                continue; // Style definition
            if (line.startsWith('click '))
                continue; // Click event
            if (line.startsWith('subgraph '))
                continue; // Subgraph
            if (line === 'end')
                continue; // Subgraph end
            // Check for valid node or edge definition
            const hasArrow = line.includes('-->') || line.includes('-.->') || line.includes('..->') || line.includes('==>');
...
//# sourceMappingURL=MermaidFormatter.js.map